...
static void tmc50xx_rampstat_work_handler(struct k_work *work)
{
			     ...
			     const struct tmc_ramp_generator_data *ramp_data)
{
	...
	err = tmc5xxx_write(config->controller, TMC50XX_VHIGH(config->index), ramp_data->vhigh);
	if (err != 0) {
		return -EIO;
	}
	err = tmc5xxx_write(config->controller, TMC50XX_VCOOLTHRS(config->index),
			    ramp_data->vcoolthrs);
	if (err != 0) {
		return -EIO;
	}
	err = tmc5xxx_write(config->controller, TMC50XX_IHOLD_IRUN(config->index),
			    ramp_data->iholdrun);
	if (err != 0) {
		return -EIO;
	}
...
static int tmc50xx_stepper_init(const struct device *dev)
{
	...
	k_work_init_delayable(&data->rampstat_callback_dwork, tmc50xx_rampstat_work_handler);
...
#define TMC50XX_DEFINE(inst)									\
	...
	DEVICE_DT_INST_DEFINE(inst, tmc50xx_init, NULL, &tmc5xxx_data_##inst,			\
